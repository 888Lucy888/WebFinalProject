...
class MatchesView extends Component {
  ...
  render() {
    let matchData =
      this.state && this.state.matches.length > 0 ? (
        this.state.matches.map((p) => (
          <MatchCard
            ...
            img={p.img}
            match={() => this.match()}
            remove={() => this.remove(p.email)}
          />
        ))
      ) : (
        <span style={{ margin: "3%", color: "aliceblue", fontSize: 20 }}>
          {" "}
          Oops! It appears you are out of matches
        </span>
      );

    return (
      <div>
        <h1 style={{ margin: "3%", fontSize: 32, color: "#1a1e55" }}>
          The Universe might like...
        </h1>
